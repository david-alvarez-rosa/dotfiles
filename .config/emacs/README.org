...
*** Calendar
Use org caldav with davmail for syncing.
#+begin_src elisp
  (use-package org-caldav
    ...
    ;; We don't actually need to sync from our org file to the calendar;
    ;; I expect to use meetings.a.c and other mechanisms to schedule
    ;; meetings
    (setq org-caldav-sync-direction 'cal->org)


    ;; Eagerly load org-agenda, since our gnus-icalendar bits break without this
    ;; (require 'org-agenda)


    ;; Configure org-caldav-sync to sync in the background after 1
    ;; minute of idle time
    ;; (defun jmi/caldav-sync-idle-timer-wrapper ()
      ;; (org-caldav-sync))

    ;; (run-with-idle-timer 60 t 'jmi/caldav-sync-idle-timer-wrapper)
