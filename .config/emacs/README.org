...
then use a hook to lower threshold back after Emacs startup.  The
loading time can be check with the function =emacs-init-time=.  Now I'm
using the Garbage Collector Magic Hack (=gcmh=) package.
#+begin_src elisp
  (use-package gcmh
    :demand t
    :init
