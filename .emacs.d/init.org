...
- Open it with Emacs and press the function =C-c C-v C-t= for tangling all the
  ...
  =init.el=.
- Move =init.el= file into =~/.emacs.d/= directory (if it does not exist, you
...
Also I have bind =C-c v= to save current window configuration and =C-v V= to
delete it. This configuration can be search with same switch buffer ivy
function.
...
- Reply to Email: template for capturing emails I need to reply to (with link
  of course). This is inside my =Tasks.org= file under "Reply to Email"
  heading.
- Read Task: read something. I usually capture tasks within Elfeed (i.e., mark
  an entry for reading later). This is inside my =Tasks.org= file under "Read"
  heading.
- Link Tasks: similar to standalone tasks, but with link (e.g., email or Elfeed
  link). This is inside my =Tasks.org= file under "Refile" heading.
...
#+begin_src emacs-lisp
  (setq org-capture-templates
        '(("t" "Task" entry
          ...
          ("e" "Email" entry
           (file+olp "~/Documents/Tasks.org" "Reply to Email")
           "* TODO [#C] Reply: %a\n %?" :empty-lines 1)
          ("r" "Read" entry
           (file+olp "~/Documents/Tasks.org" "Read")
           "* TODO [#C] Read: %a\n %?" :empty-lines 1)
          ("l" "Link Task" entry
           (file+olp "~/Documents/Tasks.org" "Refile")
           "* TODO [#C] %? %a" :empty-lines 1)
