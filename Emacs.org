...
- Functions and variables defined by me are prefixed with my name and a slash,
  i.e., =dalv/= to namespace them and avoid conflicts.
...
#+begin_src emacs-lisp
  (defun dalv/kill-all-other-buffers ()
  ...
  (global-set-key (kbd "C-c k") 'dalv/kill-all-other-buffers)
#+end_src
...
#+begin_src emacs-lisp
  (defun dalv/split-and-follow-horizontally ()
    (interactive)
    (split-window-below)
    (balance-windows)
    (other-window 1))
  (global-set-key (kbd "C-x 2") 'dalv/split-and-follow-horizontally)

  (defun dalv/split-and-follow-vertically ()
    (interactive)
    (split-window-right)
    (balance-windows)
    (other-window 1))
  (global-set-key (kbd "C-x 3") 'dalv/split-and-follow-vertically)
#+end_src
...
#+begin_src emacs-lisp
  (setq dalv/themes '(doom-one doom-one-light doom-zenburn))
  (setq dalv/themes-index 0)

  (defun dalv/cycle-theme ()
    "Cycle through themes defined in dalv/themes variable."
    (interactive)
    ;; Disable current themes.
    (mapc #'disable-theme custom-enabled-themes)
    ;; Load new theme.
    (setq dalv/themes-index (% (1+ dalv/themes-index) (length dalv/themes)))
    (setq dalv/theme (nth dalv/themes-index dalv/themes))
    (load-theme dalv/theme t)
    ;; Resets powerline.
    (when (fboundp 'powerline-reset)
      (powerline-reset)))
  (global-set-key (kbd "C-c d") 'dalv/cycle-theme)
#+end_src
...
Some configuration concerning the modeline: show battery status and time in
24hr format. I have disabled the load average and I have enabled displaying
also the date.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun dalv/toggle-transparency ()
  ...
  (define-key global-map (kbd "C-c t") 'dalv/toggle-transparency)
#+end_src
...
- Shopping: add something to my personal shopping list (as a checkitem). This
  is inside my =Tasks.org= file under "Shopping" heading.
  ...
  (setq org-capture-templates
        '(("t" "Task" entry
          ...
          ("s" "Shopping" checkitem
           (file+olp "~/Documents/Tasks.org" "Shopping" "Refile")
           "" :empty-lines-after 1)
          ("n" "Text Note" entry
...
#+begin_src emacs-lisp
  (defun dalv/org-agenda-delete-empty-blocks ()
  ...
  (add-hook 'org-agenda-finalize-hook #'dalv/org-agenda-delete-empty-blocks)
#+end_src
...
check out my [[file:~/.mbsyncrc.org][isync configuration]].
...
#+begin_src emacs-lisp :tangle no
  ...
  (defun dalv/add-encryption-mark-if-possible ()
    "Add MML tag to encrypt message when there is a key for each
  recipient."
    (when (dalv/can-encrypt-message-p)
      (mml-secure-message-sign-encrypt)))

  (add-hook 'message-send-hook #'dalv/add-encryption-mark-if-possible)
#+end_src
...
stored in =dalv/contact-file=. This text file has contacts in the form "Name
...
#+begin_src emacs-lisp
  (setq dalv/contact-file "~/Documents/Contactos/Correos.txt")
  (defun dalv/read-contact-list ()
    (with-temp-buffer
      (insert-file-contents dalv/contact-file)
      (split-string (buffer-string) "\n" t)))
  (defun dalv/complete-emails (&optional start)
    (interactive)
    ...
    (when (and eoh (> eoh (point)) (mail-abbrev-in-expansion-header-p))
      (let* ((end (point))
        ...
        (ivy-read "Contact: "
                  (dalv/read-contact-list)
                  :action (lambda(contact) (with-ivy-window (insert contact)))
                  :initial-input initial-input)))))
  (define-key mu4e-compose-mode-map (kbd "<M-tab>") 'dalv/complete-emails)
#+end_src
...
* TODO Music Player
*NOTE:* This is an old configuration (I'm still using it, but is obsolete). I'm
thinking of using EMMS in the future as =mpd= client.
...
#+begin_src emacs-lisp
  (defun dalv/elfeed-play-with-mpv ()
    (interactive)
  ...
  (define-key elfeed-search-mode-map (kbd "v") 'dalv/elfeed-play-with-mpv)
#+end_src

Ignore current entry (i.e., tag it with =no= TAG).
#+begin_src emacs-lisp
  (defun dalv/elfeed-ignore ()
    (interactive)
    ...
    (forward-line))
  (define-key elfeed-search-mode-map (kbd "i") 'dalv/elfeed-ignore)
#+end_src
...
:LOGBOOK:
- State "TODO"       from              [2021-08-24 Tue 18:53]
:END:

Take into account that this section maps a bunch of bindings involving the
=Super= key, and might interfere with you Window Manager (WM) and, therefore if
you are not thinking in using Emacs as a WM, don't tangle any block from this
section.

Important!! Take a look to [[https://github.com/Ambrevar/dotfiles/blob/master/.emacs.d/lisp/init-exwm.el][this configuration]], that is explained in this [[https://www.reddit.com/r/emacs/comments/6huok9/exwm_configs/][reddit]].
#+begin_src emacs-lisp
  (use-package exwm
    :config
    ...
    (exwm-input-set-key (kbd "s-<tab>") 'switch-to-prev-buffer)
    (exwm-input-set-key (kbd "s-<iso-lefttab>") 'switch-to-next-buffer)
